...
class DbManager {
    ...
    getUserById(id, withPassword = false) {
        const query = `SELECT ${withPassword ? '*' : 'id, email, profile_img, created_at'} FROM user WHERE id = ?;`;
        const values = [id];
        return this.executeQuery(query, values);
    }

    getUserByEmail(email, withPassword = false) {
        const query = `SELECT ${withPassword ? '*' : 'id, email, profile_img, created_at'} FROM user WHERE email = ?;`;
        const values = [email];
        return this.executeQuery(query, values);
    }

    getUserByEmailOrId(str, withPassword = false) {
        const query = `SELECT ${withPassword ? '*' : 'id, email, profile_img, created_at'} FROM user WHERE id = ? OR email = ?;`;
